<?php
namespace PWCC\WhiteListHTML;

// Taken wholesale from https://github.com/humanmade/whitelist-html.
 ...
 */
function get_whitelist_html( $text, $allowedtags = [], $context = '' ) {
	...
	foreach ( $allowedtags as $key => $tag ) {
		...
		if ( ! is_string( $tag ) ) {
			...
			_doing_it_wrong( 'whitelist_html', '$allowedtags must consist of strings or kses-style arrays', '1.0' );
			continue;
		}
 ...
 * @return string             Escaped string for output into HTML context.
 */
function the_whitelist_html( $text, $allowedtags = [], $context = '' ) {
	echo get_whitelist_html( $text, $allowedtags, $context );
}
